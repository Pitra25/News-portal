...
func WithFakeCategory(t *testing.T, dbo orm.DB, in *db.Category) Cleaner {
	if in.Title == "" {
		in.Title = cutS(gofakeit.Sentence(), 255)
	}
...
func WithFakeNews(t *testing.T, dbo orm.DB, in *db.News) Cleaner {
	...
	if in.Author == "" {
		in.Author = cutS(gofakeit.Sentence(), 255)
	}
	...
	if in.PublishedAt.IsZero() {
		in.PublishedAt = gofakeit.DateRange(time.Now().Add(5*time.Minute), time.Now().Add(1*time.Hour))
	}
